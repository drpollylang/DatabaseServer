...
public class Token {
    /* TODO:
        Token type: keyword, alterationType, symbol, booleanOperator, comparator, integerLiteral, floatLiteral, booleanLiteral, charLiteral, stringLiteral, plainText
        Token value: ...
     */
    private TokenType tokenType;
    private String tokenValue;
    final static List<String> keywords = Arrays.asList("use", "create", "drop", "alter", "insert", "select", "update",
            "delete", "join", "database", "table", "into", "values", "from", "where", "set", "on");
    ...
    private static boolean isFloat(String token) {
        ...
        if (!(isDigitSequence(splitToken[0]) && isDigitSequence(splitToken[1]))) return false;
    ...
    // Potential errors:
    // malformed query (not compliant with BNF grammar)
    // reserved keyword used as database/table/attribute name
    //
}
